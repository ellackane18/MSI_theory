...
def class_generator(cleaned_file, keep_file=True):
    ...
    repeated_att = 0
    curr_species = (
        Mess_class.Computation_Cond()
    )  # the first calss has to be computation conditions
    section_order = []
    files_to_copy = []
    ...
    for line in lines:
        ...
        if "Model" == key:
            ...
            if (
                not "WellDepth" in key
                and not "WellCutoff" in key
                and not "WellExtension" in key
            ):
                save_flag = True
                prev_species = copy.deepcopy(curr_species)
