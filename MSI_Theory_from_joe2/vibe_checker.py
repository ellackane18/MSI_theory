...
class Vibe_Checker:
    ...
    for i,p in enumerate(pressure_list):
        ...
        axs[0].semilogy(temperature_list,k2[i],color='r',label='gonzalez')
        axs[0].semilogy(temperature_list,k[i],'--',color='b',label='mess')
        # plt.xlim(1000,2500)
    ...
    fig.savefig('Vibe_Checker_N2O+O_N2+O2.pdf',dpi=1000,bbox_inches='tight')